...
| mid_pose_group_number | positive | PASS | 97.283 | >= 70.0 | landmark | True/True | True | - | 单帧 pose landmark 组为数字时应回退到手部语义评分。 |
...
| mid_pose_group_number | positive | PASS | 81.958 | >= 70.0 | landmark | True/True | True | - | 单帧 pose landmark 组为数字时应回退到手部语义评分。 |
| malformed_sidecar_ignored | positive | PASS | 100.000 | >= 95.0 | landmark | True/True | True | - | semantic_frame_weights.json 顶层错类型时应忽略 sidecar 并保持正常评分。 |
