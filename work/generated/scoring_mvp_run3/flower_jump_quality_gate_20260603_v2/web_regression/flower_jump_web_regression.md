...
- 5080 状态：worker=`ready`，worker_pid=`811485`，reload_count=`8`，last_reload_error=`None`
- 口径：不重新运行 Holistic；query 使用已保存网页/API Holistic JSON，standard 使用当前标准库。

## 结论

- 回归状态：`FAIL`
- replay 报告：`work/generated/scoring_mvp_run3/flower_jump_quality_gate_20260603_v2/web_regression/active_template_replay/web_replay_current.md`
- diagnostics 报告：`work/generated/scoring_mvp_run3/flower_jump_quality_gate_20260603_v2/web_regression/flower_jump_diagnostics/web_semantic_diagnostics.md`
...
| effective_rate_total | PASS | rate=96.1%, threshold=95.0% |
| effective_rate_花 | FAIL | rate=94.6%, reliable=92, normal_or_borderline=87, low=5 |
| effective_rate_跳 | PASS | rate=100.0%, reliable=37, normal_or_borderline=37, low=0 |
| jump_effective_low_zero | PASS | effective_low=0 |
| flower_effective_low_bounded | PASS | effective_low=5, max=5, diagnoses={'flower_core_hand_presence_low': 1, 'flower_opening_guard_failed': 4} |
| flower_effective_low_explained | FAIL | allowed=['flower_opening_guard_failed'], observed={'flower_core_hand_presence_low': 1, 'flower_opening_guard_failed': 4} |

## 全量网页回放

- 样本数 `168`，错误 `0`，正常 `102`，边界 `22`，低分 `44`。
- 旧均分 `35.442`，当前均分 `60.410`。

| 词条 | 样本数 | 正常 | 边界 | 低分 | 当前均分 | 手部覆盖均值 |
|---|---:|---:|---:|---:|---:|---:|
| 月亮 | 1 | 0 | 0 | 1 | 21.343 | 0.800 |
| 汽车 | 3 | 0 | 0 | 3 | 16.905 | 0.833 |
| 花 | 93 | 81 | 6 | 6 | 74.513 | 0.708 |
...
- 花/跳样本 `149`，错误 `0`，有效采集 `129`，有效正常+边界 `124`，有效低分 `5`，有效正常+边界率 `96.1%`。

| 词条 | 原始样本 | 建议重采 | 有效采集 | 有效正常+边界 | 有效低分 | 有效率 | 有效均分 | 诊断 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| 花 | 93 | 1 | 92 | 87 | 5 | 94.6% | 75.161 | {'flower_core_accepted': 87, 'flower_core_hand_presence_low': 1, 'flower_opening_guard_failed': 5} |
...
| web_20260522_231259_51a8c719 | 花 | 20.468 | score_valid | flower_core_hand_presence_low | opening_guard_too_weak | 0.000/0.500 | 0.450 |
