...
|---|---|---:|---|---:|---|
| 花 | PASS | 77.955 | combo_slow_sparse_freeze_lowres | 69.472 | diagnostic_quantized_micro_jitter |
...
|---|---|---|---:|---|---|---|---|---|
| combo_aspect_lowres_rate | positive | PASS | 79.288 | >= 70.0 | 53 / 1.00 | score_valid:score_valid | short_visible_core:query_not_short_core_capture | 轻微宽高比失真、低分辨率坐标取整和轻微采样间隔不均叠加。 |
...
| diagnostic_quantized_micro_jitter | diagnostic | PASS | 69.472 | diagnostic | 53 / 1.00 | score_valid:score_valid | short_visible_core:query_not_short_core_capture | 320x240 量化与较强采样抖动叠加，`花` 会贴近阈值，只记录边界。 |
...
| diagnostic_quantized_micro_jitter | diagnostic | PASS | 97.115 | diagnostic | 19 / 1.00 | score_valid:score_valid | action_window_net:used | 320x240 量化与较强采样抖动叠加，`花` 会贴近阈值，只记录边界。 |
