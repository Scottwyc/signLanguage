...
| 跳 | FAIL | 69.389 | ordered_jitter | 47.277 | swap_halves |
...
|---|---|---|---:|---|---:|---|---|---|
| reverse_full | negative | PASS | 14.024 | <= 50.0 或重采/语义失败 | 1.000 | semantic_mismatch | flower_opening_guard_failed | 完整倒放，动作语义起终点和方向相反。 |
| swap_halves | negative | PASS | 28.941 | <= 50.0 或重采/语义失败 | 1.000 | semantic_mismatch | flower_opening_guard_failed | 前后半段错序，语义相位不连续。 |
| scramble_three_phases | negative | PASS | 33.723 | <= 50.0 或重采/语义失败 | 1.000 | semantic_mismatch | flower_opening_guard_failed | 结束、中段、开始三相位乱序。 |
| ordered_jitter | positive | PASS | 79.410 | >= 70.0 | 1.000 | score_valid | score_valid | 轻微采样抖动但整体相位顺序不变。 |
...
| 变体 | 类型 | 状态 | 分数 | 阈值 | 长度比 | capture_quality | reason | 说明 |
|---|---|---|---:|---|---:|---|---|---|
| reverse_full | negative | PASS | 20.525 | <= 50.0 或重采/语义失败 | 1.000 | semantic_mismatch | phase_endpoint_order_mismatch | 完整倒放，动作语义起终点和方向相反。 |
| scramble_three_phases | negative | PASS | 25.521 | <= 50.0 或重采/语义失败 | 1.000 | semantic_mismatch | phase_endpoint_order_mismatch | 结束、中段、开始三相位乱序。 |
| swap_halves | negative | PASS | 47.277 | <= 50.0 或重采/语义失败 | 1.000 | semantic_mismatch | phase_endpoint_order_mismatch | 前后半段错序，语义相位不连续。 |
| ordered_jitter | positive | FAIL | 69.389 | >= 70.0 | 1.000 | score_valid | score_valid | 轻微采样抖动但整体相位顺序不变。 |
