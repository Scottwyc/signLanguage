...
- 均分：`65.006`
- 中位数：`77.612`
- 分段计数：`{'borderline': 22, 'normal_like': 95, 'low': 28}`
- 诊断计数：`{'flower_core_accepted': 82, 'flower_core_hand_presence_low': 5, 'flower_opening_guard_failed': 4, 'jump_core_accepted': 35, 'jump_two_hand_presence_low': 19}`
- 采集质量计数：`{'score_valid': 117, 'needs_recapture': 25, 'semantic_mismatch': 3}`
- 有效采集口径：可评分样本 `120`，正常+边界 `117`，低分 `3`，正常+边界率 `97.5%`。

## 分词条

| 词条 | 样本数 | 正常 | 边界 | 低分 | 均分 | 中位数 | 最低 | 最高 | 核心覆盖均值 | L/R 覆盖均值 | 采集质量 | 主要诊断 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|
| 花 | 91 | 75 | 7 | 9 | 73.330 | 78.401 | 4.202 | 80.784 | 0.704 | 0.017/0.698 | score_valid:82, needs_recapture:6, semantic_mismatch:3 | flower_core_accepted:82, flower_core_hand_presence_low:5, flower_opening_guard_failed:4 |
...
- 低分数：`9`
- 诊断分布：`{'flower_core_hand_presence_low': 5, 'flower_opening_guard_failed': 4}`
