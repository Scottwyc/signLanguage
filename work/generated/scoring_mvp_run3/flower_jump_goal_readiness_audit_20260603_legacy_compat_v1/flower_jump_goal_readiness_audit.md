...
- 注：本次显式允许 `legacy_frame_slice_metadata` 作为完成证据；该模式仅用于旧后端兼容复核，正式 live 复测应优先使用 `client_source=browser_camera` 或非均匀 `frame_weights` strong 证据。
