...
- 5080 状态：worker=`ready`，worker_pid=`811485`，reload_count=`28`，last_reload_error=`None`
...
| 变体 | 类型 | 状态 | 分数 | 阈值 | 长度比 | alignment | capture_quality | reason | 说明 |
|---|---|---|---:|---|---:|---|---|---|---|
| jump_relation_core_slow_1.55x_diagnostic | diagnostic | DIAG | 23.502 | diagnostic | 1.316 | semantic_action_window | needs_recapture | jump_two_hand_presence_low | 跳的核心段强拉伸会触发两手关系覆盖边界，只记录诊断。 |
...
| jump_relation_core_slow_1.40x | positive | PASS | 75.484 | >= 70.0 | 1.211 | semantic_action_window | score_valid | score_valid | 词义核心段更慢，局部帧更密但语义顺序不变。 |
