...
- 5080 状态：worker=`ready`，pid=`811485`，reload_count=`20`，last_reload_error=`None`
...
|---|---|---|---:|---|---|---|---:|---|---|---|---|---|
| all_pose_full_nan_diagnostic | diagnostic | DIAG | 23.221 | diagnostic | True | 0/0 | 5247 | full | score_valid:score_valid | short_visible_core:query_not_short_core_capture | - | 诊断记录：全段 pose 归一化信息不可用时，打分应保持有限并依靠手部证据或重采诊断。 |
...
| pose_face_sparse_nan_inf | positive | PASS | 77.581 | >= 70.0 | True | 0/0 | 63 | sparse_every_7f | score_valid:score_valid | short_visible_core:query_not_short_core_capture | - | 非核心 pose/face 稀疏坏点应被当作缺失，不应污染手部语义评分。 |
...
| 变体 | 类型 | 状态 | 分数 | 阈值 | finite | vector/mask 非有限 | 改动值 | pattern | capture_quality | semantic_floor | 异常 | 说明 |
|---|---|---|---:|---|---|---|---:|---|---|---|---|---|
| all_pose_full_nan_diagnostic | diagnostic | DIAG | 8.288 | diagnostic | True | 0/0 | 1881 | full | semantic_mismatch:weak_same_direction_vertical_jump | action_window_net:weak_same_direction_vertical_jump | - | 诊断记录：全段 pose 归一化信息不可用时，打分应保持有限并依靠手部证据或重采诊断。 |
