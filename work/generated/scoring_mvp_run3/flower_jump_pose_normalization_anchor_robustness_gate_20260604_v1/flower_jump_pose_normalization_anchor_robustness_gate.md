...
- 目标：稀疏坏肩点使用相邻可信锚点插值；整段肩部或 pose 不可信时，从有效手部中心与掌尺度回退，不能把正确手部动作打成误低分。
...
|---|---|---:|---|---:|---:|
| 花 | PASS | 78.039 | left_shoulder_xy_outlier_full_hand_fallback | 70.000 | 90.000 |
| 跳 | FAIL | 76.227 | left_shoulder_xy_outlier_sparse_interpolated | 70.000 | 90.000 |
...
| left_shoulder_xy_outlier_sparse_interpolated | PASS | 98.760 | 90.000 | True | 16 | score_valid:score_valid | 稀疏有限肩部 x/y 离群应由相邻可信肩锚点插值，不能制造归一化抖动。 |
| left_shoulder_z_outlier_sparse_interpolated | PASS | 98.760 | 90.000 | True | 8 | score_valid:score_valid | 稀疏有限肩部 z 离群应由相邻可信肩锚点插值。 |
| duplicate_shoulders_sparse_interpolated | PASS | 98.760 | 90.000 | True | 24 | score_valid:score_valid | 稀疏肩点塌缩不能把 shoulder scale 压到近零。 |
| extreme_shoulder_span_sparse_interpolated | PASS | 98.760 | 90.000 | True | 16 | score_valid:score_valid | 稀疏异常肩宽不能缩放手部与双手关系特征。 |
| vertical_shoulder_shift_sparse_interpolated | PASS | 98.760 | 90.000 | True | 16 | score_valid:score_valid | 稀疏肩部整体漂移应由可信相邻帧插值。 |
...
| vertical_shoulder_shift_full_hand_fallback | PASS | 78.039 | 70.000 | True | 106 | score_valid:score_valid | 整段肩部整体漂移时，应拒绝越界锚点并使用手部 fallback。 |
...
|---|---|---:|---:|---|---:|---|---|
| self_reloaded | PASS | 100.000 | 95.000 | True | 0 | score_valid:score_valid | 原始标准 JSON 重载后应保持近满分。 |
| left_shoulder_xy_outlier_sparse_interpolated | FAIL | 76.227 | 90.000 | True | 6 | score_valid:score_valid | 稀疏有限肩部 x/y 离群应由相邻可信肩锚点插值，不能制造归一化抖动。 |
| left_shoulder_z_outlier_sparse_interpolated | FAIL | 76.227 | 90.000 | True | 3 | score_valid:score_valid | 稀疏有限肩部 z 离群应由相邻可信肩锚点插值。 |
| duplicate_shoulders_sparse_interpolated | FAIL | 76.227 | 90.000 | True | 9 | score_valid:score_valid | 稀疏肩点塌缩不能把 shoulder scale 压到近零。 |
| extreme_shoulder_span_sparse_interpolated | FAIL | 76.227 | 90.000 | True | 6 | score_valid:score_valid | 稀疏异常肩宽不能缩放手部与双手关系特征。 |
| vertical_shoulder_shift_sparse_interpolated | FAIL | 76.227 | 90.000 | True | 6 | score_valid:score_valid | 稀疏肩部整体漂移应由可信相邻帧插值。 |
...
| vertical_shoulder_shift_full_hand_fallback | PASS | 78.935 | 70.000 | True | 38 | score_valid:score_valid | 整段肩部整体漂移时，应拒绝越界锚点并使用手部 fallback。 |
