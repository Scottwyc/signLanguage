...
| 项 | 状态 | 细节 |
|---|---|---|
| 5080/Holistic | `PASS` | worker=`ready`，pid=`811485`，reload_count=`32`，last_reload_error=`None` |
| watcher | `PASS` | event=`no_target_samples`，pid=`2311546`，target_count=`0` |
| 前端契约 | `PASS` | failed=`0`，warning=`3` |
| 综合质量门 | `PASS` | 报告 `work/generated/scoring_mvp_run3/flower_jump_quality_gate_20260604_partial_hand_bone_length_74gate_v1/flower_jump_quality_gate.md` |
| 浏览器证据门 | `PASS` | 报告 `work/generated/scoring_mvp_run3/flower_jump_retest_readiness_live_20260604_partial_hand_bone_length_74gate_postwatch_v1/browser_evidence_gate/flower_jump_browser_evidence_gate.md` |
| 网页上传权重语义 | `PASS` | checks=`8`，failed=`-` |
| 网页上传权重仿真 | `PASS` | cases=`3`，报告 `work/generated/scoring_mvp_run3/flower_jump_retest_readiness_live_20260604_partial_hand_bone_length_74gate_postwatch_v1/browser_upload_weight_simulation_gate/browser_upload_weight_simulation_gate.md` |
...
|---|---:|---|
| 花 | 100.000 | self_reloaded |
| 跳 | 100.000 | self_reloaded |
...
|---|---:|---|
| 花 | 78.039 | left_shoulder_xy_outlier_full_hand_fallback |
| 跳 | 76.227 | left_shoulder_xy_outlier_sparse_interpolated |

| 词条 | Landmark 长度正向最低分 | 最弱长度损坏 | 核心手诊断最高分 | 最强核心手长度损坏 |
|---|---:|---|---:|---|
| 花 | 78.039 | pose_drop_first_full_hand_fallback | 1.164 | right_hand_drop_first_full_recapture |
| 跳 | 76.227 | pose_drop_first_sparse_masked | 2.161 | left_hand_drop_first_full_recapture |

- Hand Wrist 正常证据审计：files=`178`，hand_frames=`4750`，violations=`0`，max_abs_z0=`9.218072136718547e-07`。

| 词条 | Wrist 身份正向最低分 | 最弱身份变体 | 整段身份损坏诊断最高分 | 最强身份损坏 |
|---|---:|---|---:|---|
| 花 | 79.773 | right_hand_landmarks_adjacent_chain_swap_preserved | 1.164 | right_hand_landmarks_rotate_left_1_full_recapture |
| 跳 | 81.642 | right_hand_landmarks_rotate_left_1_sparse_masked | 2.161 | left_hand_landmarks_rotate_left_1_full_recapture |

- Hand 内部拓扑正常证据审计：files=`178`，hand_frames=`4750`，violations=`0`，max_backtrack=`5`，max_reversed_chains=`4`，min_proximal_distal_ratio=`0.5209601104389956`。

| 词条 | 内部拓扑正向最低分 | 最弱拓扑变体 | 整段拓扑损坏诊断最高分 | 最强拓扑损坏 |
|---|---:|---|---:|---|
| 花 | 79.773 | right_hand_landmarks_adjacent_chain_swap_preserved | 1.164 | right_hand_landmarks_rotate_internal_1_full_recapture |
| 跳 | 81.642 | right_hand_landmarks_reverse_internal_all_sparse_masked | 2.161 | left_hand_landmarks_reverse_internal_all_full_recapture |

- Hand Landmark 碰撞正常证据审计：files=`178`，hand_frames=`4750`，raw_collisions=`0`，raw_quantization_signatures=`0`，min_pair_distance=`0.00014560438285116106`。

| 词条 | 碰撞正向最低分 | 最弱碰撞变体 | 严重碰撞诊断最高分 | 最强碰撞损坏 |
|---|---:|---|---:|---|
| 花 | 78.512 | right_hand_landmarks_xyz_1_256_raw_quantized | 10.731 | right_hand_landmarks_tip_to_dip_all_full_semantic_mismatch |
| 跳 | 70.714 | right_hand_landmarks_single_index_tip_to_dip_sparse_preserved | 9.307 | left_hand_landmarks_index_middle_to_index_mcp_full_semantic_mismatch |

- Hand 骨段长度正常证据审计：files=`178`，hand_frames=`4750`，segments=`71250`，violations=`0`，normal_ratio_range=`0.006013596835018576`–`1.7403748621373394`，partial_visibility=`{'patterns': {'single_missing': {'pattern_count': 21, 'evaluated_case_count': 95000, 'skipped_case_count': 4750, 'violation_count': 0, 'mask_violation_count': 0, 'minimum_bone_length_ratio': 0.006002826351681463, 'maximum_bone_length_ratio': 1.8311995705984159, 'passed': True}, 'pair_missing': {'pattern_count': 210, 'evaluated_case_count': 878750, 'skipped_case_count': 118750, 'violation_count': 0, 'mask_violation_count': 0, 'minimum_bone_length_ratio': 0.006002826351681463, 'maximum_bone_length_ratio': 1.8311995705984159, 'passed': True}, 'common_multi_missing': {'pattern_count': 8, 'evaluated_case_count': 23750, 'skipped_case_count': 14250, 'violation_count': 0, 'mask_violation_count': 0, 'minimum_bone_length_ratio': 0.006013596835018576, 'maximum_bone_length_ratio': 1.7403748621373394, 'passed': True}}, 'passed': True}`。

| 词条 | 骨段正向最低分 | 最弱骨段变体 | 整段骨段损坏诊断最高分 | 最强骨段损坏 |
|---|---:|---|---:|---|
| 花 | 78.512 | right_hand_landmarks_xyz_1_256_quantized_preserved | 10.759 | right_hand_landmarks_all_distal_long_plus_thumb_tip_out_of_bounds_full_recapture |
| 跳 | 70.714 | right_hand_landmarks_single_index_tip_short_sparse_preserved | 6.762 | left_hand_landmarks_all_edges_short_full_recapture |
...
|---|---:|---|---:|---|
| 花 | 69.388 | flower_full_left_ghost_from_right_offset | 29.506 | flower_middle45_right_core_ghost_from_left_diagnostic |
| 跳 | 70.469 | single_mid_left_duplicates_right | 19.222 | jump_full_right_ghost_from_left_diagnostic |

| 词条 | 手部融合正向最低分 | 最弱正向融合 | 手部融合诊断最低分 | 最弱诊断边界 |
|---|---:|---|---:|---|
| 花 | 81.343 | flower_right_hand_self_overlap_0.12 | 95.607 | flower_middle35_right_self_overlap_0.45_diagnostic |
| 跳 | 70.469 | single_mid_right_blend_toward_left_0.45 | 81.566 | jump_middle35_right_person_blend_left_0.55_diagnostic |

| 词条 | 手腕掌根漂移正向最低分 | 最弱正向漂移 | 手腕掌根漂移诊断最低分 | 最弱诊断边界 |
|---|---:|---|---:|---|
| 花 | 97.283 | flower_middle20_right_mcp_anchor_xy_0.029 | 94.444 | flower_middle35_right_wrist_xy_0.145_diagnostic |
| 跳 | 76.497 | jump_middle20_right_person_mcp_anchor_y_0.020 | 84.692 | jump_middle35_right_person_palm_anchor_xy_0.090_diagnostic |

| 词条 | 手指链延迟正向最低分 | 最弱正向延迟 | 手指链延迟诊断最低分 | 最弱诊断边界 |
|---|---:|---|---:|---|
| 花 | 94.483 | flower_right_all_distal_sparse_delay_2f_every_5f | 40.342 | flower_right_all_distal_advance_1f_diagnostic |
| 跳 | 71.202 | jump_right_person_distal_delay_2f | 74.355 | jump_right_person_distal_delay_4f_diagnostic |

| 词条 | 手指扇形几何正向最低分 | 最弱正向扇形漂移 | 手指扇形几何诊断最低分 | 最弱诊断边界 |
|---|---:|---|---:|---|
| 花 | 81.220 | flower_right_nonoverlap_full_compress_0p08 | 63.377 | flower_right_full_fan_full_cross_1p20_diagnostic |
| 跳 | 76.629 | jump_right_person_sparse_compress_0p20_every_5f | 82.302 | jump_right_person_full_cross_1p20_diagnostic |

| 词条 | 手指基座几何正向最低分 | 最弱正向基座漂移 | 手指基座几何诊断最低分 | 最弱诊断边界 |
|---|---:|---|---:|---|
| 花 | 81.347 | flower_right_nonoverlap_base_full_compress_0p05 | 78.227 | flower_right_full_base_full_cross_1p40_diagnostic |
| 跳 | 98.112 | jump_right_person_base_full_compress_0p12 | 82.302 | jump_right_person_base_full_cross_1p60_diagnostic |

| 词条 | 手指链软置信正向最低分 | 最弱正向低置信 | 手指链软置信诊断最低分 | 最弱诊断边界 |
|---|---:|---|---:|---|
| 花 | 100.000 | flower_right_all_fingers_confidence_0p85_full | 100.000 | flower_right_all_fingers_confidence_0p51_middle35_diagnostic |
| 跳 | 99.987 | jump_left_ground_fingers_confidence_0p65_middle20 | 100.000 | jump_right_person_fingers_confidence_0p51_middle35_diagnostic |

| 词条 | 手指链时间平滑正向最低分 | 最弱正向平滑 | 手指链时间平滑诊断最低分 | 最弱诊断边界 |
|---|---:|---|---:|---|
| 花 | 80.010 | flower_right_all_distal_3tap_strength_0p35_full | 55.790 | flower_right_all_distal_heavy5_strength_1p00_full_diagnostic |
| 跳 | 77.745 | jump_left_ground_distal_3tap_strength_0p35_full | 72.654 | jump_right_person_distal_heavy5_strength_0p85_middle35_diagnostic |

| 词条 | 非有限坐标正向最低分 | 最弱正向坏点 | 非有限坐标诊断最低分 | 最弱诊断边界 |
|---|---:|---|---:|---|
| 花 | 98.946 | pose_face_sparse_nan_inf | 78.039 | all_pose_full_nan_diagnostic |
| 跳 | 70.714 | jump_right_person_tip_single_nan | 78.935 | all_pose_full_nan_diagnostic |

| 词条 | 有限异常/退化坐标正向最低分 | 最弱正向坏点 | 有限异常/退化坐标诊断最低分 | 最弱诊断边界 |
|---|---:|---|---:|---|
| 花 | 99.312 | flower_right_outer_tips_sparse_out_of_frame | 37.624 | flower_right_core_hand_middle35_out_diagnostic |
| 跳 | 70.714 | jump_right_person_tips_single_out_of_frame | 0.236 | jump_right_core_hand_full_duplicate_wrist_diagnostic |
...
- 完成度审计：`work/generated/scoring_mvp_run3/flower_jump_retest_readiness_live_20260604_partial_hand_bone_length_74gate_postwatch_v1/goal_readiness/flower_jump_goal_readiness_audit.md`
- 前端契约：`work/generated/scoring_mvp_run3/flower_jump_retest_readiness_live_20260604_partial_hand_bone_length_74gate_postwatch_v1/frontend_contract/watch_status_frontend_contract.md`
- 浏览器证据门：`work/generated/scoring_mvp_run3/flower_jump_retest_readiness_live_20260604_partial_hand_bone_length_74gate_postwatch_v1/browser_evidence_gate/flower_jump_browser_evidence_gate.md`
- 网页上传权重仿真：`work/generated/scoring_mvp_run3/flower_jump_retest_readiness_live_20260604_partial_hand_bone_length_74gate_postwatch_v1/browser_upload_weight_simulation_gate/browser_upload_weight_simulation_gate.md`
- 综合质量门：`work/generated/scoring_mvp_run3/flower_jump_quality_gate_20260604_partial_hand_bone_length_74gate_v1/flower_jump_quality_gate.md`
