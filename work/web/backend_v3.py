#!/usr/bin/env python3
"""V3 frontend shell for the signLanguage scoring MVP.

This version keeps port 5080 as the single shared Holistic scoring backend.
Port 5082 serves the countdown frontend and local reference videos, while
/api/status, /api/templates, and /api/score are proxied to 5080.
"""
...
STATIC_DIR = Path(__file__).resolve().parent / "static_v3"
OUTPUT_ROOT = base.WORK_DIR / "generated/web_scoring_mvp_v3"
DEMO_VIDEO_ROOT = base.REPO_ROOT / "data/Demo词汇视频/Demo词汇视频"
PORT = 5082
SHARED_BACKEND_BASE = "http://127.0.0.1:5080"

app = FastAPI(title="signLanguage Scoring MVP V3", version="0.3.0")
app.add_middleware(
    CORSMiddleware,
...
def api_status() -> Dict[str, Any]:
    ...
    status["service"] = "signLanguage scoring MVP V3"
    ...
    status["ui_version"] = "v3_countdown_reference_video"
    status["port"] = PORT
...
def api_score(request: base.ScoreRequest) -> Dict[str, Any]:
    ...
    result["frontend_proxy"] = {
        "version": "v3_countdown_reference_video",
        "port": PORT,
