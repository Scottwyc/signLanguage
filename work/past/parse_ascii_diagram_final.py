...
if os.path.exists(SIMHEI_PATH):
    chinese_font = FontProperties(fname=SIMHEI_PATH, size=8)
else:
    chinese_font = None
...
def find_box_from_corner(lines, top, left, max_height):
    ...
    # 向右找右上角 ┐
    right = None
    for c in range(left + 1, min(width, left + 150)):
        if c >= len(line):
            break
        char = line[c]
        if char == '┐':
    ...
    return {
        ...
        'text_lines': [l for l in text_lines if l.strip() and not all(c in '─═' for c in l.strip())]
    }
    ...
    for box in sorted_boxes:
        ...
        if box['width'] > 60 and box['height'] > 20:
            ...
            if box['text_lines']:
                ...
                # 计算文本位置
                text_y_start = y1 - height * 0.15
                text_step = height / (len(display_lines) + 1)
                
                for i, line_text in enumerate(display_lines):
                    text_y = text_y_start - text_step * i
                    ax.text((x1 + x2) / 2, text_y, line_text.strip(),
...
if __name__ == '__main__':
    ...
    for md_file in md_files:
        if os.path.exists(md_file):
            output = md_file.replace('.md', '_ascii_chart_final.png')
