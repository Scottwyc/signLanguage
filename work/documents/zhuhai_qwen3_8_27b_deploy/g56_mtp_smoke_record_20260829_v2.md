...
| **主管side**（3de5d01b，SignL3） | g29/8050 | ✅ n=2 | ⚠️ 仅 1 条 user_message 事件，无最终产物 | 未完成 / 未真正产出 |

- **动画side（无MTP）率先产出完整动画 + 3 张真实渲染截图**；主管side（MTP）events 停滞（直到观察结束仍只有任务指令）。
- > 注：主管side（3de5d01b）连 g29 但 13+ 分钟 events 未增长，可能未真正启动生成（详见复盘），不能完全归因于 MTP；但 g29 槽位速率全程低于 g56 是实打实的 8096 数据（见①）。

**③ 路由确认（11435 代理 route 日志，权威归属）**

- 动画任务请求**密集打在 g56**（`model=qwen3.8-27b-int4-tp2-g56`，ts≈1788025488-1788026409 连续多条）——**动画side 用的就是无MTP槽位**。
- g29 仅有零星请求（ts≈1788025107/1788025143/1788025620），与主管side 停滞一致。

**④ 结论补充**

- **量化证实**：真实高熵前端动画任务下，**无 MTP（g56，~48 tok/s）比有 MTP（g29，~31 tok/s）快约 1.5×**——MTP 的投机验证开销在不可预测的代码/动画生成中**超过**省 token 的收益，成为净负优化。
- 产物侧也印证：**无 MTP 的动画side率先完成并渲染截图**；有 MTP 的主管side在观察期内未能产出。
- **这进一步支持 §13.4 建议**：成员以高熵代码 / 前端动画 / 创意任务为主时，**MTP 应回退或不起用**。
